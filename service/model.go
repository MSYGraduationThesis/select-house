...
func Service房产投资模型(总价, 总面积, 首付比例, 契税费率, 印花税, 租金, 贷款年限, 贷款利率, 物业费用月单价, 理财利率, 推理年限 float64) interface{} {
	...
	money = money.计算投入成本增长()
	fmt.Println(fmt.Sprintf("计算投入成本增长:%.5f ", money.投入合计))
	money = money.计算理财收入()
